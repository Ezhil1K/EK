...
def article(slug):
    ...
    # 1. Check if the article slug exists in the data structure
    if slug in data.get('articles', {}):
        article_data = data['articles'][slug]
        
        # 2. CRITICAL FIX: Explicitly check for the article template name.
        if slug == 'cleanliness-risks':
             # Renders the specific file with the content you created
             return render_template('article_cleanliness_risks.html', data=data, article=article_data)
        
        # Fallback for all other articles (like 'vci-vs-oil')
        return render_template('article_template.html', data=data, article=article_data)


    # If the slug is not found in the data, return 404
